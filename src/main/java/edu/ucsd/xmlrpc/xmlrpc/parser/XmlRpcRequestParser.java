 ...
 * under the License.    
 */
package edu.ucsd.xmlrpc.xmlrpc.parser;
...
import edu.ucsd.xmlrpc.xmlrpc.common.TypeFactory;
import edu.ucsd.xmlrpc.xmlrpc.common.XmlRpcStreamConfig;


/** A SAX parser for an {@link edu.ucsd.xmlrpc.xmlrpc.client.XmlRpcClient}'s
 * request.
 */
public class XmlRpcRequestParser extends RecursiveTypeParserImpl {
		...
		switch (level++) {
			...
			case 1:
				if (methodName == null) {
					...
					if ("".equals(pURI)  &&  "ID".equals(pLocalName)) {
						inJobID = true;
					} else {
						throw new SAXParseException("Expected ID element, got "
								+ new QName(pURI, pLocalName),
								getDocumentLocator());
					}
				} else {
			...
			case 1:
				if (inMethodName) {
					...
					if ("".equals(pURI)  &&  "ID".equals(pLocalName)) {
						...
						throw new SAXParseException("Expected /ID, got "
													+ new QName(pURI, pLocalName),
													getDocumentLocator());
					}
					inJobID = false;
	...
	/** Returns the job ID of the request.
